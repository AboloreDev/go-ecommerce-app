...
func LoadEnv() (*Config, error) {
	...
	jwtExpiresAt, _ := time.ParseDuration(getEnv("JWT_EXPIRES_IN", "24h"))
	...
	return &Config{
		...
		AWSServices: AWSConfig{
			...
			Key: getEnv("AWS_SECRE_ACCESS_KEY", "test"),
			KeyId: getEnv("AWS_ACCESS_KEY_ID", "test"),
			S3Bucket: getEnv("AWS_S3_BUCKET", "armory_db_bucket"),
		...
		Uploads: UploadConfig{
			Uplaod: getEnv("UPLOAD_DIR", "/tmp/uploads"),
